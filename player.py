        ...
        if self.cash >= cost:
            ...
            if success_rate > 5:
                ...
                if self.cash >= cost_prod:
                    ...
                    print(
                        f"-Information about aircraft, name {aircraft.name}, capacity {aircraft.capacity}, range {aircraft.range}, fuel consumption {aircraft.fuel_consumption}, "
                        f"speed {aircraft.speed}, develpment cost {aircraft.development_cost}, production cost {aircraft.production_cost}.")
                else:
                    print(
                        f"-The company {self.company} does not have the necessary funds to produce "
                        f"aircraft {self.cash}$, cost of produce is {cost_prod}$")
        else:
            print(
                f"-The company {self.company} does not have the necessary funds to develop "
