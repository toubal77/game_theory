...
def decision_strategie(coop):
    ...
    if random.choice([1, 0]):
        ...
        if random.choice([1, 0]):
            ...
            coopera = coop(cash_cooperation)
            print(f"-cash of players before cooperation toubal is {toubal.cash} and oualladi {oualladi.cash}")
            # choice type of plane airbus
            if random.choice([1, 0]):
                airbus = aircraft.Aircraft("Airbus", 0, 0, 0, 0, 0, True)
                coopera.develop_aircraft(airbus, toubal, oualladi)
            else:
                boeing = aircraft.Aircraft("Boeing", 0, 0, 0, 0, 0, False)
                coopera.develop_aircraft(boeing, toubal, oualladi)
...
if __name__ == '__main__':
    ...
    # Simuler le processes de development pour les avions en cours de développement
    simulation_processus()

    # Demander aux joueurs de prendre des décisions stratégiques pour leur entreprise
    decision_strategie(cooperation.Cooperation)
